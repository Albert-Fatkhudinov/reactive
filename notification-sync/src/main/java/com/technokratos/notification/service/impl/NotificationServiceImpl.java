...
public class NotificationServiceImpl implements NotificationService {
    ...
    public List<NotificationDTO> getNotifications(RequestPerson person) {
        List<NotificationDTO> taxs = taxRepository.findAllByPersonName(person.getPersonName()).stream()
                .map(mapper::toNotification)
                .collect(Collectors.toList());
        List<NotificationDTO> fines = fineRepository.findAllByPersonName(person.getPersonName()).stream()
                .map(mapper::toNotification)
                .collect(Collectors.toList());
